export default function PhysicsRenderer(size, shader, renderer) {
...
PhysicsRenderer.prototype.render = function (scene, camera, target) {
  var _t = renderer.getRenderTarget();
  renderer.setRenderTarget(target);
  renderer.render(scene, camera);
  renderer.setRenderTarget(_t);

};

PhysicsRenderer.prototype.pass = function (shader, target) {
  this.mesh.material = shader;
  var _t = this.renderer.getRenderTarget();
  this.renderer.setRenderTarget(target);
  this.renderer.render(this.scene, this.camera);
  this.renderer.setRenderTarget(_t);
};
...
PhysicsRenderer.prototype.setUniforms = function (uniforms) {
  ...
  this.simulation.uniforms.t_pos = {
    type: "t",
    value: null
  };
  this.simulation.uniforms.t_oPos = {
    type: "t",
    value: null
  };
  this.simulation.uniforms.resolution = {
    type: "v2",
    value: this.resolution
  };
